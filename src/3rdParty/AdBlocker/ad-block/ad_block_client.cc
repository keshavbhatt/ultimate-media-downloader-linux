...
public:
  ...
  uint64_t operator()(const char *input, int len, unsigned char lastCharCode,
                      uint64_t lastHash) override;
...
bool isBadFingerprint(const char *fingerprint, const char *fingerprintEnd) {
  for (unsigned int i = 0;
       i < sizeof(badFingerprints) / sizeof(badFingerprints[0]); i++) {
...
bool hasBadSubstring(const char *fingerprint, const char *fingerprintEnd) {
  for (unsigned int i = 0; i < sizeof(badSubstrings) / sizeof(badSubstrings[0]);
       i++) {
...
void parseFilter(const char *input, Filter *f, BloomFilter *bloomFilter,
  ...
  parseFilter(input, end, f, bloomFilter, exceptionBloomFilter,
              hostAnchoredHashSet, hostAnchoredExceptionHashSet,
...
void parseFilter(const char *input, const char *end, Filter *f,
                 BloomFilter *bloomFilter, BloomFilter *exceptionBloomFilter,
                 HashSet<Filter> *hostAnchoredHashSet,
                 HashSet<Filter> *hostAnchoredExceptionHashSet,
                 HashSet<CosmeticFilter> *simpleCosmeticFilters,
                 bool preserveRules) {
  ...
  } else if (exceptionBloomFilter && (f->filterType & FTException) &&
             (f->filterType & FTHostOnly)) {
...
AdBlockClient::AdBlockClient()
    : filters(nullptr), cosmeticFilters(nullptr), htmlFilters(nullptr),
      exceptionFilters(nullptr), noFingerprintFilters(nullptr),
      ...
      noFingerprintAntiDomainOnlyExceptionFilters(nullptr), numFilters(0),
      numCosmeticFilters(0), numHtmlFilters(0), numExceptionFilters(0),
      numNoFingerprintFilters(0), numNoFingerprintExceptionFilters(0),
      ...
      numHostAnchoredFilters(0), numHostAnchoredExceptionFilters(0),
      bloomFilter(nullptr), exceptionBloomFilter(nullptr),
      hostAnchoredHashSet(nullptr), hostAnchoredExceptionHashSet(nullptr),
      ...
      badFingerprintsHashSet(nullptr), numFalsePositives(0),
      numExceptionFalsePositives(0), numBloomFilterSaves(0),
      numExceptionBloomFilterSaves(0), numHashSetSaves(0),
      numExceptionHashSetSaves(0), deserializedBuffer(nullptr) {}

AdBlockClient::~AdBlockClient() { clear(); }
...
void discoverMatchingPrefix(BadFingerprintsHashSet *badFingerprintsHashSet,
                            const char *str, BloomFilter *bloomFilter,
...
bool isNoFingerprintDomainHashSetMiss(HashSet<NoFingerprintDomain> *hashSet,
  ...
  while (start != host) {
    if (*(start - 1) == '.') {
      if (hashSet->Find(NoFingerprintDomain(
              start, static_cast<int>(host + hostLen - start)))) {
        return false;
      }
    }
    start--;
  ...
  return !hashSet->Find(
      NoFingerprintDomain(start, static_cast<int>(host + hostLen - start)));
}

bool AdBlockClient::isHostAnchoredHashSetMiss(
    const char *input, int inputLen, HashSet<Filter> *hashSet,
    const char *inputHost, int inputHostLen, FilterOption contextOption,
    const char *contextDomain, Filter **foundFilter) {
  if (!hashSet) {
    return false;
  }
  ...
  while (start != inputHost) {
    if (*(start - 1) == '.') {
      Filter *filter = hashSet->Find(
          Filter(start, static_cast<int>(inputHost + inputHostLen - start),
                 nullptr, start, inputHostLen - (start - inputHost)));
      if (filter &&
          filter->matches(input, inputLen, contextOption, contextDomain)) {
        if (filter->tagLen == 0 ||
            tagExists(std::string(filter->tag, filter->tagLen))) {
          if (foundFilter) {
            *foundFilter = filter;
  ...
  Filter *filter = hashSet->Find(
      Filter(start, static_cast<int>(inputHost + inputHostLen - start), nullptr,
             start, inputHostLen));
  if (!filter) {
    return true;
  }
...
bool AdBlockClient::matches(const char *input, FilterOption contextOption,
  ...
  if (contextDomain && !(contextOption & (FOThirdParty | FONotThirdParty))) {
    if (isThirdPartyHost(contextDomain, contextDomainLen, inputHost,
                         static_cast<int>(inputHostLen))) {
  ...
  if (!isNoFingerprintDomainHashSetMiss(noFingerprintDomainHashSet,
                                        contextDomain, contextDomainLen)) {
    hasMatch =
        hasMatch ||
  ...
  if (isNoFingerprintDomainHashSetMiss(noFingerprintAntiDomainHashSet,
                                       contextDomain, contextDomainLen)) {
    hasMatch =
        hasMatch || hasMatchingFilters(noFingerprintAntiDomainOnlyFilters,
                                       numNoFingerprintAntiDomainOnlyFilters,
                                       input, inputLen, contextOption,
                                       contextDomain, &inputBloomFilter,
                                       inputHost, inputHostLen, matchedFilter);
  }

  hasMatch = hasMatch ||
             hasMatchingFilters(noFingerprintFilters, numNoFingerprintFilters,
                                input, inputLen, contextOption, contextDomain,
                                &inputBloomFilter, inputHost, inputHostLen,
                                matchedFilter);
  ...
  if (!hasMatch) {
    ...
    hostAnchoredHashSetMiss = isHostAnchoredHashSetMiss(
        input, inputLen, hostAnchoredHashSet, inputHost, inputHostLen,
  ...
  if (!hasMatch && !bloomFilterMiss) {
    hasMatch = hasMatchingFilters(
        filters, numFilters, input, inputLen, contextOption, contextDomain,
        &inputBloomFilter, inputHost, inputHostLen, matchedFilter);
  ...
  // Only bother checking the no fingerprint domain related filters if needed
  if (!isNoFingerprintDomainHashSetMiss(noFingerprintDomainExceptionHashSet,
                                        contextDomain, contextDomainLen)) {
    hasExceptionMatch =
        hasExceptionMatch ||
  ...
  if (isNoFingerprintDomainHashSetMiss(noFingerprintAntiDomainExceptionHashSet,
                                       contextDomain, contextDomainLen)) {
    hasExceptionMatch =
        hasExceptionMatch ||
  ...
  hasExceptionMatch =
      ...
      hasMatchingFilters(noFingerprintExceptionFilters,
                         ...
                         contextOption, contextDomain, &inputBloomFilter,
                         inputHost, inputHostLen, matchedExceptionFilter);
  ...
  bool bloomExceptionFilterMiss =
      exceptionBloomFilter && !exceptionBloomFilter->substringExists(
                                  input, AdBlockClient::kFingerprintSize);
  bool hostAnchoredExceptionHashSetMiss = isHostAnchoredHashSetMiss(
      input, inputLen, hostAnchoredExceptionHashSet, inputHost, inputHostLen,
      contextOption, contextDomain, matchedExceptionFilter);
  ...
  if (!bloomExceptionFilterMiss) {
    if (!hasMatchingFilters(exceptionFilters, numExceptionFilters, input,
      ...
      if (badFingerprintsHashSet) {
        discoverMatchingPrefix(badFingerprintsHashSet, input,
...
bool AdBlockClient::findMatchingFilters(const char *input,
  ...
  *matchingExceptionFilter = nullptr;
  ...
  hasMatchingFilters(noFingerprintFilters, numNoFingerprintFilters, input,
                     inputLen, contextOption, contextDomain, nullptr, inputHost,
                     inputHostLen, matchingFilter);

  if (!*matchingFilter) {
    hasMatchingFilters(noFingerprintDomainOnlyFilters,
                       numNoFingerprintDomainOnlyFilters, input, inputLen,
                       contextOption, contextDomain, nullptr, inputHost,
                       inputHostLen, matchingFilter);
  }
  if (!*matchingFilter) {
  ...
  if (!*matchingFilter) {
    hasMatchingFilters(filters, numFilters, input, inputLen, contextOption,
                       contextDomain, nullptr, inputHost, inputHostLen,
                       matchingFilter);
  }

  if (!*matchingFilter) {
    isHostAnchoredHashSetMiss(input, inputLen, hostAnchoredHashSet, inputHost,
                              inputHostLen, contextOption, contextDomain,
                              matchingFilter);
  }
  ...
  hasMatchingFilters(noFingerprintExceptionFilters,
                     numNoFingerprintExceptionFilters, input, inputLen,
                     contextOption, contextDomain, nullptr, inputHost,
                     inputHostLen, matchingExceptionFilter);

  if (!*matchingExceptionFilter) {
    hasMatchingFilters(noFingerprintDomainOnlyExceptionFilters,
                       numNoFingerprintDomainOnlyExceptionFilters, input,
                       inputLen, contextOption, contextDomain, nullptr,
                       inputHost, inputHostLen, matchingExceptionFilter);
  }
  ...
  if (!*matchingExceptionFilter) {
    isHostAnchoredHashSetMiss(input, inputLen, hostAnchoredExceptionHashSet,
                              inputHost, inputHostLen, contextOption,
                              contextDomain, matchingExceptionFilter);
  }

  if (!*matchingExceptionFilter) {
    hasMatchingFilters(exceptionFilters, numExceptionFilters, input, inputLen,
                       contextOption, contextDomain, nullptr, inputHost,
                       inputHostLen, matchingExceptionFilter);
  }
...
void AdBlockClient::initBloomFilter(BloomFilter **pp, const char *buffer,
                                    int len) {
  if (*pp) {
    delete *pp;
  }
...
bool AdBlockClient::parse(const char *input, bool preserveRules) {
  ...
  cout << "Num new no fingerprint filters: " << newNumNoFingerprintFilters
       << endl;
  cout << "Num new no fingerprint exception filters: "
       << newNumNoFingerprintExceptionFilters << endl;
  cout << "Num new host anchored filters: " << newNumHostAnchoredFilters
       << endl;
  ...
  Filter *newHtmlFilters = new Filter[newNumHtmlFilters + numHtmlFilters];
  ...
  while (true) {
    if (isEndOfLine(*p) || *p == '\0') {
      ...
      parseFilter(lineStart, p, &f, bloomFilter, exceptionBloomFilter,
                  hostAnchoredHashSet, hostAnchoredExceptionHashSet,
                  &simpleCosmeticFilters, preserveRules);
      if (!f.hasUnsupportedOptions()) {
        switch (f.filterType & FTListTypesMask) {
        case FTException:
          if (f.filterType & FTHostOnly) {
...
#ifdef PERF_STATS
  cout << "Simple cosmetic filter size: " << simpleCosmeticFilters.GetSize()
       << endl;
#endif
...
// written or needed
int serializeFilters(char *buffer, size_t bufferSizeAvail, Filter *f,
                     int numFilters) {
  char sz[256];
  int bufferSize = 0;
  for (int i = 0; i < numFilters; i++) {
    int sprintfLen =
        snprintf(sz, sizeof(sz), "%x,%x,%x", static_cast<int>(f->filterType),
...
char *AdBlockClient::serialize(int *totalSize, bool ignoreCosmeticFilters,
  ...
  *totalSize +=
      ...
      snprintf(
          ...
          numFilters, numExceptionFilters, adjustedNumCosmeticFilters,
          adjustedNumHtmlFilters, numNoFingerprintFilters,
          ...
          noFingerprintDomainHashSetSize, noFingerprintAntiDomainHashSetSize,
          noFingerprintDomainExceptionHashSetSize,
          noFingerprintAntiDomainExceptionHashSetSize);
  *totalSize +=
      serializeFilters(nullptr, 0, filters, numFilters) +
      serializeFilters(nullptr, 0, exceptionFilters, numExceptionFilters) +
      serializeFilters(nullptr, 0, cosmeticFilters,
                       adjustedNumCosmeticFilters) +
      serializeFilters(nullptr, 0, htmlFilters, adjustedNumHtmlFilters) +
      serializeFilters(nullptr, 0, noFingerprintFilters,
                       numNoFingerprintFilters) +
      serializeFilters(nullptr, 0, noFingerprintExceptionFilters,
                       numNoFingerprintExceptionFilters) +
      serializeFilters(nullptr, 0, noFingerprintDomainOnlyFilters,
                       numNoFingerprintDomainOnlyFilters) +
      serializeFilters(nullptr, 0, noFingerprintAntiDomainOnlyFilters,
                       numNoFingerprintAntiDomainOnlyFilters) +
  ...
  *totalSize += noFingerprintAntiDomainExceptionHashSetSize;
  ...
  pos += serializeFilters(buffer + pos, *totalSize - pos, exceptionFilters,
                          numExceptionFilters);
...
bool AdBlockClient::deserialize(char *buffer) {
  ...
  sscanf(buffer + pos,
         "%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x",
         &numFilters, &numExceptionFilters, &numCosmeticFilters,
         &numHtmlFilters, &numNoFingerprintFilters,
         &numNoFingerprintExceptionFilters, &numNoFingerprintDomainOnlyFilters,
         &numNoFingerprintAntiDomainOnlyFilters,
         &numNoFingerprintDomainOnlyExceptionFilters,
         &numNoFingerprintAntiDomainOnlyExceptionFilters,
  ...
  pos +=
      deserializeFilters(buffer + pos, exceptionFilters, numExceptionFilters);
  pos += deserializeFilters(buffer + pos, cosmeticFilters, numCosmeticFilters);
  pos += deserializeFilters(buffer + pos, htmlFilters, numHtmlFilters);
  pos += deserializeFilters(buffer + pos, noFingerprintFilters,
                            numNoFingerprintFilters);
  pos += deserializeFilters(buffer + pos, noFingerprintExceptionFilters,
                            numNoFingerprintExceptionFilters);

  pos += deserializeFilters(buffer + pos, noFingerprintDomainOnlyFilters,
                            numNoFingerprintDomainOnlyFilters);
  pos += deserializeFilters(buffer + pos, noFingerprintAntiDomainOnlyFilters,
                            numNoFingerprintAntiDomainOnlyFilters);
  pos +=
      deserializeFilters(buffer + pos, noFingerprintDomainOnlyExceptionFilters,
                         numNoFingerprintDomainOnlyExceptionFilters);
  ...
  pos += bloomFilterSize;
  initBloomFilter(&exceptionBloomFilter, buffer + pos,
                  exceptionBloomFilterSize);
  pos += exceptionBloomFilterSize;
  if (!initHashSet(&hostAnchoredHashSet, buffer + pos,
                   hostAnchoredHashSetSize)) {
    return false;
  }
  pos += hostAnchoredHashSetSize;
  if (!initHashSet(&hostAnchoredExceptionHashSet, buffer + pos,
                   hostAnchoredExceptionHashSetSize)) {
    return false;
  }
  pos += hostAnchoredExceptionHashSetSize;

  if (!initHashSet(&noFingerprintDomainHashSet, buffer + pos,
                   noFingerprintDomainHashSetSize)) {
    return false;
  }
  pos += noFingerprintDomainHashSetSize;

  if (!initHashSet(&noFingerprintAntiDomainHashSet, buffer + pos,
                   noFingerprintAntiDomainHashSetSize)) {
    return false;
  }
  pos += noFingerprintAntiDomainHashSetSize;

  if (!initHashSet(&noFingerprintDomainExceptionHashSet, buffer + pos,
                   noFingerprintDomainExceptionHashSetSize)) {
    return false;
  }
  pos += noFingerprintDomainExceptionHashSetSize;

  if (!initHashSet(&noFingerprintAntiDomainExceptionHashSet, buffer + pos,
                   noFingerprintAntiDomainExceptionHashSetSize)) {
    return false;
  }
  pos += noFingerprintAntiDomainExceptionHashSetSize;
