...
NoFingerprintDomain::NoFingerprintDomain()
    : borrowed_data(false), data(nullptr), dataLen(-1) {}
...
NoFingerprintDomain::NoFingerprintDomain(const char *data, int dataLen)
    : borrowed_data(true), data(const_cast<char *>(data)), dataLen(dataLen) {}
...
uint32_t NoFingerprintDomain::Serialize(char *buffer) {
  ...
  char sz[64];
  uint32_t dataLenSize = 1 + snprintf(sz, sizeof(sz), "%xx", dataLen);
  if (buffer) {
