...
Filter::Filter()
    : borrowed_data(false), filterType(FTNoFilterType),
      filterOption(FONoFilterOption), antiFilterOption(FONoFilterOption),
      ruleDefinition(nullptr), data(nullptr), dataLen(-1), domainList(nullptr),
      tag(nullptr), tagLen(0), host(nullptr), hostLen(-1), domains(nullptr),
...
Filter::Filter(const char *data, int dataLen, char *domainList,
               const char *host, int hostLen, char *tag, int tagLen)
    : borrowed_data(true), filterType(FTNoFilterType),
      filterOption(FONoFilterOption), antiFilterOption(FONoFilterOption),
      ruleDefinition(nullptr), data(const_cast<char *>(data)), dataLen(dataLen),
      domainList(domainList), tag(tag), tagLen(tagLen),
      host(const_cast<char *>(host)), hostLen(hostLen), domains(nullptr),
      antiDomains(nullptr), domainsParsed(false) {}

Filter::Filter(FilterType filterType, FilterOption filterOption,
               FilterOption antiFilterOption, const char *data, int dataLen,
               char *domainList, const char *host, int hostLen, char *tag,
               int tagLen)
    : borrowed_data(true), filterType(filterType), filterOption(filterOption),
      ...
      data(const_cast<char *>(data)), dataLen(dataLen), domainList(domainList),
      tag(tag), tagLen(tagLen), host(const_cast<char *>(host)),
      hostLen(hostLen), domains(nullptr), antiDomains(nullptr),
      domainsParsed(false) {}
...
void Filter::parseOption(const char *input, int len) {
  ...
  if (len >= 7 && !strncmp(pStart, "domain=", 7)) {
    ...
    *pFilterOption = static_cast<FilterOption>(*pFilterOption | FOThirdParty);
    ...
    // Same as ~third-party
    *pFilterOption =
        static_cast<FilterOption>(*pFilterOption | FONotThirdParty);
    ...
    *pFilterOption =
        static_cast<FilterOption>(*pFilterOption | FOExplicitCancel);
  } else {
...
bool Filter::matchesOptions(const char *input, FilterOption context,
  ...
  if (context & (FOThirdParty | FONotThirdParty)) {
    if ((filterOption & FOThirdParty) && (context & FONotThirdParty)) {
      return false;
    }
    if ((antiFilterOption & FOThirdParty) && (context & FOThirdParty)) {
      return false;
    }
  }
 ...
 */
int indexOfFilter(const char *input, int inputLen, const char *filterBegin,
                  const char *filterEnd) {
  const int filterLen = filterEnd - filterBegin;
  if (1 == filterLen && '^' == *filterBegin)
    return -1;
...
bool Filter::matches(const char *input, int inputLen,
                     ...
                     BloomFilter *inputBloomFilter, const char *inputHost,
                     int inputHostLen) {
  ...
  if (filterType & FTHostAnchored) {
    ...
    if (host) {
      hostLen =
          this->hostLen == -1 ? static_cast<int>(strlen(host)) : this->hostLen;
    }
  ...
  while (filterPartStart != filterPartEnd || *filterPartStart == '*') {
    ...
    if (inputBloomFilter) {
      for (int i = 1;
           i < filterPartLen && filterPartEnd - filterPartStart - i >= 2; i++) {
...
uint32_t Filter::Serialize(char *buffer) {
  ...
  char sz[64];
  uint32_t dataLenSize =
      1 + snprintf(sz, sizeof(sz), "%x,%x,%x,%x", dataLen, filterType,
